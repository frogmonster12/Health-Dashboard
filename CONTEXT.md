...
| AI Insights (optional) | Gemini Flash (via `gemini-flash-latest` alias, direct browser → Gemini API, user's own key — insights only, not parsing) |
...
3. **Gemini API key never touches the Worker** — Gemini is used only for AI insights (direct browser → Gemini API). Parsing always goes through the Cloudflare Worker regardless of whether a key is present.
...
3. On Analyze: always calls `POST /analyze` on the Cloudflare Worker (Llama 3.1 8B). All files run in parallel via `Promise.allSettled`. Each card updates independently as results arrive.
...
- `generateInsights(panels)` → makes a **single** `_geminiCall` with `responseSchema` (Gemini structured-output mode). The model returns one JSON object keyed by section (`overview`, `renal`, `lipid`, …). Each section's text is written to `state.insights` via `_updateTab` and triggers `renderActiveTab` if that tab is active. Failed or empty sections render with `.insight-error` styling — the spinner never hangs. 1 call per dashboard render vs 7–8 previously, keeping free-tier headroom large.
...
- `generateInsights()` fires a single Gemini call covering all sections at once
- All sections become available together when the call resolves
...
| Extracted text | Always sent to Cloudflare Worker `/analyze` only. Gemini receives only aggregated marker summaries (never raw PDF text) when AI insights are enabled. |
...
5. **Gemini model deprecations** — Google retired 1.5 Flash in early 2026 and 2.0 Flash shuts down June 1, 2026. We use the `gemini-flash-latest` alias to track the current stable model automatically. If insights start failing with a 404, check Google's model availability page first before touching any code.
6. **Gemini free-tier limits** — As of 2026-05-17, `gemini-flash-latest` resolves to Gemini 3 Flash (5 RPM / 20 RPD free tier). The single-call insight design preserves headroom: 1 call/render vs 7–8 previously. If Google tightens limits further or the alias retargets again, check `ai.google.dev/pricing`.
